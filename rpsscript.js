...
function playGame(playerChoice) {
 ...
 if (playerHearts <= 0) {
        ...
        if (playerHearts < hearts.length) {
            hearts[playerHearts].style.display = 'none'; // Hide the heart image
        }
    }
...
function showScoreboard() {
    ...
    /*scoreboard*/
    if (topScores.length < 5 || playerScore > Math.min(...topScores.map(score => score.score))) {
        if (topScores.length >= 5) {
            topScores.pop(); // Remove the lowest score
        }
        topScores.push({ name: name || "Anonymous", score: playerScore });
    ...
    scoreboardText.textContent = `Good Work! ${name || "fella"}, ${playerScore} points!`;
